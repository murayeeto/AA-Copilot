    ...
    try:
        ...
        if "where did i land" in prompt.lower():
            ...
            if flights:
                ...
                response = f"Your last flight landed at {arrival_airport}." if arrival_airport else "I could not determine your last flight's arrival airport."
            else:
                response = "You don't have any saved flights."
            ...
            # Forward query to OpenAI for general questions
            flight_context = "\n".join([
                f"Flight {f.get('flight_number')} from {f.get('departure_airport')} to {f.get('arrival_airport')}, arriving at {f.get('arrival_time')}."
                for f in flights
            ]) if flights else "No saved flights available."

            openai_prompt = f"User's saved flights:\n{flight_context}\n\nUser's question: {prompt}"
            openai_response = chat_with_assistant(openai_prompt)
...
def itinerary():
    try:
        ...
        if action == "get_last_location":
            flights = user_data.get("flights", [])
            if not flights:
                return jsonify({"error": "No saved flights found"}), 404

            last_flight = flights[-1]  # Assuming the last flight is the most recent
            arrival_airport = last_flight.get("arrival_airport")
            if arrival_airport:
                return jsonify({"location": arrival_airport}), 200
            else:
                return jsonify({"error": "No arrival airport found for the last flight"}), 404

        elif action == "generate_itinerary":
            location = data.get("location")
            duration = data.get("duration", 3)
            if not location:
                return jsonify({"error": "Location is required to generate an itinerary"}), 400

            prompt = f"Create a {duration}-day itinerary for {location}. Include landmarks, restaurants, and activities."
            itinerary = chat_with_assistant(prompt)
            return jsonify({"itinerary": itinerary}), 200

        elif action == "save_itinerary":
            location = data.get("location")
            duration = data.get("duration")
            itinerary = data.get("itinerary")
            if not all([location, duration, itinerary]):
                return jsonify({"error": "Location, duration, and itinerary are required to save"}), 400

            user_ref.update({
                "itineraries": firestore.ArrayUnion([{
                    "location": location,
                    "duration": duration,
                    "itinerary": itinerary,
                }])
        ...
        print(f"Error in /itinerary endpoint: {str(e)}")
        return jsonify({"error": str(e)}), 500
